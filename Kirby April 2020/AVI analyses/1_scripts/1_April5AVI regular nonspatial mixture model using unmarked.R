...
#2. site covariates
veg50.recast<-read.csv("0_data/processed/veg50.recast.csv",header=TRUE)
veg150.recast<-read.csv("0_data/processed/veg150.recast.csv",header=TRUE)
veg500.recast<-read.csv("0_data/processed/veg500.recast.csv",header=TRUE)
m1<-merge(veg50.recast,veg150.recast,by=c("SS"))
m2<-merge(m1,veg500.recast,by=c("SS"))
write.csv(m2, file="0_data/processed/Kirby.avi.3spatscale.csv")

site_avi_cov<-read.csv("0_data/processed/Kirby.avi.3spatscale.csv",header=TRUE)
site_avi_cov$SS<-NULL
cormat <- round(cor(site_avi_cov),2)
head(cormat)
...
sitecov<-read.csv("0_data/processed/Kirby.avi.3spatscale.csv",header=TRUE)
sitecov<-sitecov[,-1]
...
for(i in names){ 
  ...
  site.covs.wo.SS50<-sitecov[,c("Blackspruce_50",
                               "Tremblingaspen_50",
                               "Bog_50",
                               "Fen_50",
                               "Jackpine_50",
  ...
  smat50 <- outer(1:12, 1:12, vCorrelated, data = site.covs.wo.SS50)
  ...
  ggsave(paste0("3_outputs/figures/",i,"top50m_AVImodel.png"), plot=GG, width=12, height=6, units=c("in"), dpi=300)
  ...
  GG50<-tm_shape(r) +
    ...
    tm_legend(legend.outside=TRUE)
  tmap_save(GG50, width = 1000, height = 1000, units="px", filename = paste0("3_outputs/maps/BestModel50mAVIPredicted",i,".png"))
  ...
  write.csv(df50, file=paste0("3_outputs/data/PredictedAbundanceAVI50m",i,".csv"))
